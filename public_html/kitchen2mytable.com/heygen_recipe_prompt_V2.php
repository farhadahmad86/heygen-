    ...
    if ($recipe) {
        ...
        if (file_exists($localPath)) {
            ...
            if (filesize($localPath) > $maxSize) {
                ...
                $compressedPath = dirname($localPath) . '/compressed_' . basename($localPath);
        ...
        $recipePrompt = "Create a premium meal advertising short-form recipe video for Kitchen2MyTable.\n\n"
            ...
            . "  - Visual: Use the Source Image Asset URL ({$imageUrl}) as a visual reference. If the supplied image is a thumbnail, low resolution, blurry, or poor quality, you should generate a premium high-quality image closely matching the original recipe appearance, ingredients, plating style, colors, and overall presentation.\n"
            . "  - Keep the generated result visually consistent with the supplied image so it appears to be the same meal.\n"
            . "  - Display using smart 9:16 portrait fitting.\n"
            . "  - Automatically adjust scaling for landscape or small images while preserving aspect ratio.\n"
            . "  - Use a blurred enlarged version of the same visual for background fill.\n"
            . "  - Apply gentle cinematic zoom and slow panning effects.\n"
            . "  - Voiceover: \"And here is the final result — a delicious, beautifully prepared {$title}, ready to serve and enjoy with your family and friends.\"\n"
            ...
            . "    Speak the following script EXACTLY word-for-word in the exact order written below. Do NOT summarize, rewrite, shorten, paraphrase, remove words, or skip any sentence. Every word below is mandatory and must be fully narrated before ending the video:\n"
...
<body class="text-slate-100 min-h-screen py-10 px-4 md:px-6 flex items-center justify-center font-sans tracking-tight">
        ...
        <form method="post" id="genForm" class="space-y-6 <?= ($previewMode || $sessionId) ? 'hidden' : ''; ?>">
            <div class="bg-slate-900/50 rounded-2xl p-5 border border-slate-800/60 text-left space-y-4">
                <label for="recipeIdInput"
                    ...
                    ID Target</label>
                <input type="number" id="recipeIdInput" name="id" value="<?= h($recipeId ?? '') ?>" required
            ...
            <div class="bg-slate-900/50 rounded-2xl p-5 border border-slate-800/60 text-left space-y-5">
                ...
                <div class="space-y-2">
                    ...
                    <input type="url" id="customImageUrlInput" name="custom_image_url"
                        value="<?= !empty($customImageUrl) ? h($customImageUrl) : (!empty($recipeId) ? "https://kitchen2mytable.com/uploads/recipe_" . $recipeId . ".jpg" : "") ?>"
                        ...
                        placeholder="https://domain.com/path/to/recipe-image.jpg">Leave blank to inherit primary image
                    assets
    ...
    document.addEventListener("DOMContentLoaded", function() {
        // --- REAL-TIME URL AUTO-FILL ENGINE ---
        ...
        if (idInput && urlInput) {
            idInput.addEventListener('input', function() {
                ...
                if (idValue && idValue > 0) {
                    urlInput.value = `https://kitchen2mytable.com/uploads/recipe_${idValue}.jpg`;
                } else {
                    urlInput.value = '';
                }
            });
